...
setenv mu0 $argv[1]
setenv re $argv[2]
setenv sersic $argv[3]
setenv n $argv[4]
setenv mu_limit $argv[5]
setenv suffix $argv[6]

awk -v a=$mu0 -v b=$re -v c=$sersic '{print($a" 0.0 "log($b*0.05)/log(10)" 0.0 "log($c)/log(10)" 0.0")}' deimos.all.sersic.sigma.lst > deimos.all.sersic.sigma.${suffix}.lst


awk -v a=$mu0 -v b=$re -v c=$sersic '{print($a" 0.0 "log($b*0.05)/log(10)" 0.0 "log($c)/log(10)" 0.0")}' matko.all.sersic.sigma.lst > matko.all.sersic.sigma.${suffix}.lst

awk -v a=$mu0 -v b=$re -v c=$sersic '{print($a" 0.0 "log($b*0.05)/log(10)" 0.0 "log($c)/log(10)" 0.0")}' cody.all.sersic.sigma.lst > cody.all.sersic.sigma.${suffix}.lst
