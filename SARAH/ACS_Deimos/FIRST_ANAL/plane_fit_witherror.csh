...
setenv mu_e `echo $mu+1 | bc -l`
setenv re_e `echo $re+1 | bc -l`
setenv sigma_e `echo $sigma+1 | bc -l`

setenv n $argv[4]
setenv mu_limit $argv[5]
setenv suffix $argv[6]

awk -v a=$mu -v b=$re -v c=$sigma -v ae=$mu_e -v be=$re_e -v ce=$sigma_e '{print($a+2.5*log(2*3.1415*$b*$b*0.0025)/log(10)" "$ae+5.0*$be/$b/log(10)" "log($b*0.05)/log(10)" "(log($b*0.05)/log(10))*$be/$b/log(10)" "log($c)/log(10)" "(log($c)/log(10))*$ce/$c/log(10))}' deimos.all.sersic.sigma.lst > deimos.all.sersic.sigma.${suffix}.lst


awk -v a=$mu -v b=$re -v c=$sigma -v  ae=$mu_e -v be=$re_e -v ce=$sigma_e '{print($a+2.5*log(2*3.1415*$b*$b*0.0025)/log(10)" "$ae+5.0*$be/$b/log(10)" "log($b*0.05)/log(10)" "(log($b*0.05)/log(10))*$be/$b/log(10)" "log($c)/log(10)" "(log($c)/log(10))*$ce/$c/log(10))}' matko.all.sersic.sigma.lst > matko.all.sersic.sigma.${suffix}.lst


awk -v a=$mu -v b=$re -v c=$sigma -v  ae=$mu_e -v be=$re_e -v ce=$sigma_e '{print($a+2.5*log(2*3.1415*$b*$b*0.0025)/log(10)" "$ae+5.0*$be/$b/log(10)" "log($b*0.05)/log(10)" "(log($b*0.05)/log(10))*$be/$b/log(10)" "log($c)/log(10)" "(log($c)/log(10))*$ce/$c/log(10))}' cody.all.sersic.sigma.lst > cody.all.sersic.sigma.${suffix}.lst
