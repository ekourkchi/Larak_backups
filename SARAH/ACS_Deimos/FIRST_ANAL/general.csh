...
setenv psf_root ${ID}'_'${no_object}
...
setenv  psf_root 'tiny_'${ID}'_no'${no_object}
#echo $psf_root
./tinymagic.csh $psf_root  $x0  $y0


setenv check ${ID}'.segment.fits'
setenv badmap ${ID}'_'${no_object}'.badmap.'${ver}'.fits'
...
########################## GALFIT MAKER #############################
setenv galin ${ID}'_'${no_object}'.gal.in.'${ver}
if (-e ${galin}) then
rm ${galin}
endif

setenv filename 'h_v'${ID}'_F814W_drz_cl_ver2.fits'
setenv filename 'h_v'${ID}'_F814W_ivm_drz_cl_ver2.fits'
setenv file_rms 'h_v'${ID}'_F814W_rms_ver2.fits '
setenv galoutfile ${ID}'_'${no_object}'.gal.out.'${ver}'.fits'
...
echo "J) 25.937                          # Magnitude photometric zeropoint"   >> $galin
...
setenv fitout ${ID}'_'${no_object}'.galfit.'${ver}
mv fit.log $fitout
rm galfit.01



setenv bad ${ID}_${no_object}.gal.out.${ver}.badmap.fits
if (-e ${bad}) then
rm ${bad}
endif
...
setenv plfile ${ID}_${no_object}.gal.out.${ver}.galaxy.pl
if (-e ${plfile}) then
rm ${plfile}
endif
...
echo "\!rm ${ID}_${no_object}.gal.out.${ver}.galaxy.pl"  >> ${cl_iraf}
...
echo "imcopy ${badmap}   ${ID}_${no_object}.gal.out.${ver}.galaxy.pl"  >> ${cl_iraf}
echo "ellipse input='${ID}_${no_object}.gal.out.${ver}.galaxy.fits' output='"$isogal".tab' x0="$cx" y0="$cy" ellip0="$ell" pa0="$PA" sma=10 step=0.1 interac=no "  >> ${cl_iraf}
echo "ellipse input='${ID}_${no_object}.gal.out.${ver}.sersic.fits' output='"$isoser".tab' x0="$cx" y0="$cy" ellip0="$ell" pa0="$PA" sma=5 step=0.1 interac=no " >> ${cl_iraf}
