pro plot475_814_sdynamo_ML_proposal, psout, limag
...
erM_dynamics=sqrt((2*ersigma/sigma)^2+(erre814/re)^2)
...
;print, erM_Ldyn814
print, "  "
M_Ldyn475=-2.5*M_dynamics/(mag475-35.0+2.5*alog10(2.)-5.14)
;print, M_Ldyn475

erM_Ldyn475=abs(-2.5*erM_dynamics/(mag475-35.0+2.5*alog10(2.)-4.57))
;============================================
;============================================
...
ccc74=ccc74[where(modgmp3 ne 0)]
		...
		;xtitl=" "
		xtitl="log(M!DD!N/M!D!9n!N!X)"
		ytitl="log(M!DD!N/L!D814!N) [M!D!9n!N!X/L!D!9n!N!X]"
		plot, [0], /nodata, xthick=3., ythick=3.,  xtitle=xtitl, ytitle=ytitl, yrange=[0.825,1.275], xrange=[7,11], xstyle=1, ystyle=1, charsize=1.3, ytickinterval=0.1, yminor=4 

u1=8
u2=9.2
v1=0.825
v2=1.275
polyfill, [u1,u1,u2,u2,u1],[v1,v2,v2,v1,v1],color=cc[11]
plot, [0], /noerase, xthick=3., ythick=3.,  xtitle=xtitl, ytitle=ytitl, yrange=[0.825,1.275], xrange=[7,11], xstyle=1, ystyle=1, charsize=1.3, ytickinterval=0.1, yminor=4 

xyouts, 8.25,0.925, 'Proposed'
xyouts, 8.3, 0.9, 'HYDRA'
xyouts, 8.2, 0.875, 'Observation'


		;oploterror, bbb30, ccc30, bbb30*0., ddd30, psym=yoursymbol(25,color=cc[0]), /nohat, ERRCOLOR=cc[0]
		oplot, bbb30, ccc30, psym=yoursymbol(25,color=cc[0])

		;oploterror, bbb62, ccc62, bbb62*0., ddd62, psym=yoursymbol(22,color=cc[8]), color=cc[8], /nohat, ERRCOLOR=cc[8]
		oplot, bbb62, ccc62, psym=yoursymbol(22,color=cc[8])

		;oploterror, bbb74, ccc74, bbb74*0., ddd74, psym=yoursymbol(21,color=cc[30]), color=cc[30], /nohat, ERRCOLOR=cc[30]
		oplot, bbb74, ccc74, psym=yoursymbol(21,color=cc[30])



		;oplot, [7,11], b1*[7,11]+b0, linestyle=2, thick=3
...
;polyfill, [7.1,7.1,8.28,8.28],[1.07,1.17,1.17,1.07],color=cc[10]
		...
		;xyouts, 7.2,1.14,'a='+strtrim(string(b1, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db1, FORMAT='(F5.2)'),2), color=cc[50], size=1.1
		
		;xyouts, 7.2,1.09,'b='+strtrim(string(b0, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db0, FORMAT='(F5.2)'),2), color=cc[50], size=1.1
;------------------------------------------------------------------------------
;------------------------------------------------------------------------------
...
polyfill, [9.7,9.7,10.88,10.88],[0.95,0.85,0.85,0.95],color=cc[10]
		...
		xyouts, 9.8,0.92,'a='+strtrim(string(b1, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db1, FORMAT='(F5.2)'),2), color=cc[50], size=1.1
		
		xyouts, 9.8,0.87,'b='+strtrim(string(b0, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db0, FORMAT='(F5.2)'),2), color=cc[50], size=1.1

;xyouts, 8.5,0.87,'RMS='+strtrim(string(rms, FORMAT='(F5.3)'),2), size=1.2
;------------------------------------------------

resid1=m_l814-(b1*m+b0)
;------------------------------------------------------------------------------

;xyouts, 8.2,1.225,'(A)', charsize=1.1, color=cc[0]
legend, ["Deimos Sample","MG05","Co09"], color=[0,8,30] , psym=[25,22,21], /clear, charsize=0.9, /right
