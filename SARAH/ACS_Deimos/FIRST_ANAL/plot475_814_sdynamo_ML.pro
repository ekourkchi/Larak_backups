pro plot475_814_sdynamo_ML, psout, limag
...
ccc74=ccc74[where(modgmp3 ne 0)]
		...
		xtitl=" "
		ytitl="log(M!Ddyn!N/L) [(M/L)!D!9n!N!X]"
...
;------------------------------------------------------------------------------
xyouts, 7.40,1.5,'F8145W-band', size=1.30
...
xtitl=" "
ytitl="log(M!Ddyn!N/L) [(M/L)!D!9n!N!X]"
		plot,  bbb30, ccc30, psym=yoursymbol(25,color=cc[0]),  xthick=5.,ythick=5., ytitle=ytitl, yrange=[-0.6,2.6], xrange=[7,11.50], xstyle=1, ystyle=1, charsize=1.8, ytickinterval=0.5, yminor=5 
		oplot, bbb62, ccc62, psym=yoursymbol(22,color=cc[8]), color=cc[8]
		oplot, bbb74, ccc74, psym=yoursymbol(21,color=cc[30]), color=cc[30]
		;oplot, [7,11.50], b1*[7,11.50]+b0, linestyle=2, thick=3
	
		
		;xyouts, 7.2,1.19,'c='+strtrim(string(b1, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db1, FORMAT='(F5.2)'),2), color=cc[50], size=1.1
		
		;xyouts, 7.2,1.14,'d='+strtrim(string(b0, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db0, FORMAT='(F5.2)'),2), color=cc[50], size=1.1
;------------------------------------------------------------------------------
;------------------------------------------------------------------------------
...
;;print, b1,b0,db1,db0
		...
		xyouts, 9.78,-0.1,'a='+strtrim(string(b1, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db1, FORMAT='(F5.2)'),2), color=cc[50], size=1.1
;		
		xyouts, 9.7,-0.4,'b='+strtrim(string(b0, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db0, FORMAT='(F5.2)'),2), color=cc[50], size=1.1

;xyouts, 8.5,-0.50,'RMS='+strtrim(string(rms, FORMAT='(F5.3)'),2), size=1.2

xyouts, 7.40,1.7,'F475W-band', size=1.30

xyouts, 9.2,2.25,'(B)', charsize=1.3, color=cc[0]
legend, ["Deimos Sample","MG05","Co09"], color=[0,8,30] , psym=[25,22,21], /clear, charsize=1.1
;------------------------------------------------------------------------------
;------------------------------------------------------------------------------
...
;delta=g-r
;delta= (-2.5*alog10(f475_2)+26.068)-(-2.5*alog10(f814_2)+25.937)
...
;legend, item, linestyle=lstyle, color=c_colors , psym=sym, /clear, charsize=1.1, /right, /bottom
...
color=(-2.5*alog10(f475_2)+26.068)-(-2.5*alog10(f814_2)+25.937)
;color=(-2.5*alog10(f475_1)+26.068)-(-2.5*alog10(f814_1)+25.937)
;
;color=g-r
;plot, color, ser814-ser475, psym=2.
;plot, bet,alf, psym=2.
...
multiplot
;xtitl="log(M!Ddyn!N/M!D!9n!N!X)"
xtitl=""
;xtitl="!MA!X log (Dynamical Mass)"
ytitl="log(M!D*!N/L) [(M/L)!D!9n!N!X]"
plot,[0], /nodata, xrange=[7,11.50] ,yrange=[0.275,0.725], xstyle=1, ystyle=1, charsize=1.8, xthick=5.,ythick=5., ytitle=ytitl, ytickinterval=0.1, yminor=4 , xtitle=xtitl
...
ccc=Ms_LII+0.18   ; 0.18=0.45/2.5 convert vega system to AB system.
ccc30=ccc[0:30]
ccc62=ccc[31:62]
ccc74=ccc[63:74]
ccc30=ccc30[where(modgmp1 ne 0)]
...
oplot, bbb30, ccc30, psym=yoursymbol(25,color=cc[0]), color=cc[0]
oplot, bbb62, ccc62, psym=yoursymbol(22,color=cc[8]), color=cc[8]
oplot, bbb74, ccc74, psym=yoursymbol(21,color=cc[30]), color=cc[30]


		;xyouts, 1.4,0.57,'a='+strtrim(string(trend[0], FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(trend[2], FORMAT='(F5.2)'),2), color=cc[50], size=1.2
		
		;xyouts, 1.4,0.59,'b='+strtrim(string(trend[1], FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(trend[3], FORMAT='(F5.2)'),2), color=cc[50], size=1.2


xyouts, 9.2,0.67,'(C)', charsize=1.3, color=cc[0]
xyouts, 7.40,0.60,'I-band', size=1.60
oplot, [7.35+0.025,7.45+0.025], [0.618,0.618], color=cc[0]
oplot, [7.35+0.025,7.45+0.025], [0.6,0.6], color=cc[0]

oplot, bbb30, ccc30, psym=yoursymbol(25,color=cc[0]), color=cc[0]
legend, ["Deimos Sample","MG05","Co09"], color=[0,8,30] , psym=[25,22,21], /clear, charsize=1.1


xx=bbb(where(M_dynamics ge 10.6))

yy=ccc(where(M_dynamics ge 10.6))

trend=ltrend(xx,yy,0.02*yy/yy)
oplot,  [-20,20], trend[0]*[-20,20]+trend[1], linestyle=1, thick=3.
...
xresid=[bbb30,bbb62,bbb74]
yresid=[ccc30,ccc62,ccc74]
xresid=xresid(where(rM_dynamics lt 10.50))
yresid=yresid(where(rM_dynamics lt 10.50))
rresid=yresid-(b1*xresid+b0)
rms=sqrt(mean(rresid^2))
print, "RMSC:", rms


		;xyouts, 7.2,0.375,'a='+strtrim(string(b1, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db1, FORMAT='(F5.2)'),2), color=cc[50], size=1.1
		
		;;xyouts, 7.2,0.325,'b='+strtrim(string(b0, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db0, FORMAT='(F5.2)'),2), color=cc[50], size=1.1

;xyouts, 9.5,0.325,'RMS='+strtrim(string(rms, FORMAT='(F5.3)'),2), size=1.2
...
ytitl="log(M!D*!N/L!Dg!N) [(M/L)!D!9n!N!X]"
;plot,[0], /nodata, xrange=[7,11.50] ,yrange=[0.325,0.775], xstyle=1, ystyle=1, charsize=1.5, xthick=3.,ythick=3., ytitle=ytitl, ytickinterval=0.1, yminor=4 
...
ccc=Ms_Lg   ; 0.18=0.45/2.5 convert vega system to AB system.
ccc30=ccc[0:30]
ccc62=ccc[31:62]
ccc74=ccc[63:74]
ccc30=ccc30[where(modgmp1 ne 0)]
...
;oplot, bbb30, ccc30, psym=2, color=cc[0]
...
;xyouts, 8.50,0.725,'(D)', charsize=1.3, color=cc[31]

;legend, ["Deimos Sample","MG05","Co09"], color=[0,8,30] , psym=[2,4,5], /clear, charsize=1.1

xx=bbb(where(M_dynamics ge 10))
yy=ccc(where(M_dynamics ge 10))
...
;print, "RMSD:", rms
		...
		xyouts, 7.5,0.375,'a='+strtrim(string(b1, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db1, FORMAT='(F5.2)'),2), color=cc[50], size=1.2
		
		xyouts, 7.5,0.325,'b='+strtrim(string(b0, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db0, FORMAT='(F5.2)'),2), color=cc[50], size=1.2

;xyouts, 10,0.35,'RMS='+strtrim(string(rms, FORMAT='(F5.3)'),2), size=1.2
...
ccc=m_s
;ccc= modre814kpc
;bbb=M_dynamics[where(modgmp ne 0)]
bbb=M_dynamics
xtitl="log(M!D*!N/M!D!9n!N!X)"
ytitl="log(M!Ddyn!N/M!D!9n!N!X)"
		plot,  [0], /nodata, psym=yoursymbol(27,color=cc[50]),  xthick=5.,ythick=5.,   xstyle=1, ystyle=1, charsize=1.8, ytickinterval=1, yminor=4 ,  xtickinterval=1, xminor=4, ytitle=xtitl, xtitle=ytitl, yrange=[7.0,11.5], xrange=[7,11.5]
...
;xyouts, 9,8.25,'a='+strtrim(string(fit[0], FORMAT='(F6.2)'),2)+'!M+!X'+strtrim(string(fit[2],FORMAT='(F5.2)'),2), color=cc[50], size=1.6
;xyouts, 9,8.0,'b='+strtrim(string(fit[1], FORMAT='(F6.2)'),2)+'!M+!X'+strtrim(string(fit[3],FORMAT='(F5.2)'),2), color=cc[50], size=1.6
xyouts, 9,7.75,'CC='+strtrim(string(correlation([xxx],[yyy]),FORMAT='(F5.2)'),2), size=1.6
;xyouts, -16.8,0.50, 'R!De!N !M?!X L!U' +strtrim(string(trend[0]*(-2.50), FORMAT='(F6.2)'),2)+'!M+!X'+strtrim(string(trend[2]*(2.5),FORMAT='(F5.2)'),2), size=2.0
xyouts, 7.3,10.25, 'M!Ddyn!N !M?!X M!D*!N!U' +strtrim(string(trend[0], FORMAT='(F6.2)'),2), size=2.0
;oplot,  [-80,80], trend[0]*[-80,80]+trend[1], linestyle=2, thick=4.
oplot,  [-80,80], [-80,80], linestyle=2, thick=4.
...
xyouts, 9.2,11.0,'(D)', charsize=1.3, color=cc[0]
