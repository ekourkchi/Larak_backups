pro plot475_814_sdynamo_ML_color, psout, limag
...
ps_on , psout, noask='y', /color, cc=cc  , xsize=18,ysize=40, scale_factor=0.8;, 
...
ccc74=ccc74[where(modgmp3 ne 0)]
		...
		xtitl=" "
		ytitl="log(M!Ddyn!N/L!D814!N) [(M/L)!D!9n!N!X]"
		plot, [0], /nodata, xthick=3., ythick=3.,  xtitle=xtitl, ytitle=ytitl, yrange=[-1.30,2.0], xrange=[7,11.50], xstyle=1, ystyle=1, charsize=1.5, ytickinterval=0.5, yminor=5 
...
polyfill, [9.5,9.5,10.88,10.88],[0.95,0.85,0.85,0.95],color=cc[10]
	
		xyouts, 9.5,-0.70,'a='+strtrim(string(b1, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db1, FORMAT='(F5.2)'),2), color=cc[50], size=1.1
		
		xyouts, 9.5,-1.0,'b='+strtrim(string(b0, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db0, FORMAT='(F5.2)'),2), color=cc[50], size=1.1

xyouts, 7.2,1.10,'RMS='+strtrim(string(rms, FORMAT='(F5.3)'),2), size=1.2
;------------------------------------------------

resid1=m_l814-(b1*m+b0)
;------------------------------------------------------------------------------

xyouts, 8.70,1.7,'(A)', charsize=1.1, color=cc[0]
legend, ["Deimos Sample","MG05","Co09"], color=[0,8,30] , psym=[25,22,21], /clear, charsize=0.9
...
xtitl=" "
ytitl="log(M!Ddyn!N/L!D475!N) [(M/L)!D!9n!N!X]"
		plot,  bbb30, ccc30, psym=yoursymbol(25,color=cc[0]),  xthick=3.,ythick=3., ytitle=ytitl, yrange=[-0.9,2.40], xrange=[7,11.50], xstyle=1, ystyle=1, charsize=1.5, ytickinterval=0.5, yminor=5 
...
;;print, b1,b0,db1,db0
		...
		xyouts, 9.58,-0.3,'a='+strtrim(string(b1, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db1, FORMAT='(F5.2)'),2), color=cc[50], size=1.1
;		
		xyouts, 9.5,-0.6,'b='+strtrim(string(b0, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(db0, FORMAT='(F5.2)'),2), color=cc[50], size=1.1

xyouts, 7.20,1.50,'RMS='+strtrim(string(rms, FORMAT='(F5.3)'),2), size=1.2


xyouts, 8.70,2.10,'(B)', charsize=1.1, color=cc[0]
legend, ["Deimos Sample","MG05","Co09"], color=[0,8,30] , psym=[25,22,21], /clear, charsize=0.9
;------------------------------------------------------------------------------
;------------------------------------------------------------------------------
...
;legend, item, linestyle=lstyle, color=c_colors , psym=sym, /clear, charsize=0.9, /right, /bottom
...
ytitl="(m!D475!N-m!D814!N) [mag]"
plot,[0], /nodata, xrange=[7,11.50] ,yrange=[0.85,1.55], xstyle=1, ystyle=1, charsize=1.5, xthick=3.,ythick=3., ytitle=ytitl, ytickinterval=0.1, yminor=4 , xtitle=xtitl
...
xyouts, 8.70,1.475,'(C)', charsize=1.1, color=cc[0]

legend, ["Deimos Sample","MG05","Co09"], color=[0,8,30] , psym=[25,22,21], /clear, charsize=0.9


xx=bbb(where(M_dynamics ge 10.2))

yy=ccc(where(M_dynamics ge 10.2))
