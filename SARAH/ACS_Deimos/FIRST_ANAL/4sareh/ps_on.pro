...
SCALE_FACTOR=SCALE_FACTOR, SET_FONT=SET_FONT
...
;** set portrait mode, yoffset is for middle of US letter
    ...
    if n_elements(xsize) EQ 0   then xsize = 17.8
    if n_elements(ysize) EQ 0   then ysize = 12.7
...
;** set plot to postscipt and call device
  ...
  device, landscape=landscape, portrait=portrait, encapsulated=encapsulated, $
    file=file_name, font_size=font_size, color=color, SET_FONT=SET_FONT, $
    xsize=xsize, xoffset=xoffset, ysize=ysize, yoffset=yoffset, SCALE_FACTOR=SCALE_FACTOR, /bold;, /italic,/helv,/isolatin1
...
if keyword_set(color) EQ 1 then standard_colors, cc=cc, invert=invert else $
