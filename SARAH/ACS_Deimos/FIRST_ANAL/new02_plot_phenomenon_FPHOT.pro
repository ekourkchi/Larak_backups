pro new02_plot_phenomenon_FPHOT, psout
...
rms_re= stdev(delre)
rms_re=rms_re[1]
print, rms_mu, sqrt(mean(delmu*delmu)), rms_re, sqrt(mean(delre*delre))
...
rms_re=rms_re[1]
print, rms_mu, sqrt(mean(delmult21*delmult21)), rms_re, sqrt(mean(delrelt21*delrelt21))
...
rms_re=rms_re[1]
print, rms_mu, sqrt(mean(delmugt21*delmugt21)), rms_re, sqrt(mean(delregt21*delregt21))
...
xyouts, 21.0,17.3,'b='+strtrim(string(beta, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(dbeta, FORMAT='(F5.2)'),2), color=cc[0], size=1.0
xyouts, 21.0,16.9,'c='+strtrim(string(gama, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(dgama, FORMAT='(F5.2)'),2), color=cc[0], size=1.0
xyouts, 21.0,18.1,'(PHP-F814W)', color=cc[50], size=1.1
...
ytitl="<!7l!X>!De!N!U(F475W)!N [mag/arcsec!U2!N]"
...
xyouts, 21.0,18.3,'b='+strtrim(string(beta, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(dbeta, FORMAT='(F5.2)'),2), color=cc[0], size=1.0
xyouts, 21.0,17.9,'c='+strtrim(string(gama, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(dgama, FORMAT='(F5.2)'),2), color=cc[0], size=1.0
xyouts, 21.0,19.1,'(PHP-F475W)', color=cc[50], size=1.1
xyouts, 21.0,19.5,'Modified Sample', color=cc[50], size=1.1
...
;x=x[where(modgmp ne 0)]
;y=y[where(modgmp ne 0)]
...
delmu=delmu[where(modgmp ne 0)]
delre=delre[where(modgmp ne 0)]
print,  sqrt(mean(delmu*delmu)), sqrt(mean(delre*delre))



t=-1.23*logn814+0.76
re_s= (y-beta*logsigma-gama-t)/alfa
delmu=y-x-t
delre=re814kpc-re_s
delmu=delmu[where(modgmp ne 0)]
delre=delre[where(modgmp ne 0)]
