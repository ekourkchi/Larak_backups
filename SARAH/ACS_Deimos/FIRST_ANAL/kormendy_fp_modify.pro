pro kormendy_fp_modify, psout, limag
...
ps_on , psout, noask='y', /color, cc=cc  , xsize=18,ysize=16, scale_factor=0.9;, 
...
x1=aa30(where(mag814_30 lt 15))
x2=aa62(where(mag814_62 lt 15))
x3=aa74(where(mag814_74 lt 15))
y1=bb30(where(mag814_30 lt 15))
y2=bb62(where(mag814_62 lt 15))
y3=bb74(where(mag814_74 lt 15))
y=[y1,y2,y3]
x=[x1,x2,x3]
...
plot, [0], /nodata, xthick=3.,ythick=3.,charsize=1.3, xrange=[-0.5,1.6], yrange=[-21,-14] , color=cc[0],  xstyle=1, ystyle=1, xtitle="Log (R!De!N)!Dkpc!N", ytitle="<!7l!X>!De!N!UF814W!N"


oplot, x,y , psym= yoursymbol(25,color=cc[0])
item=['M!D814!N<-20']
lstyle=[1]
sym= [25]
c_colors=[0]
...
oplot, [-0.5,10.0], p*[-0.5,10.0]+q, linestyle=0, thick=4.
trend=ortltrenderror( x, y, 0.05*x, 0.1*y/y, 0.05, 0.05, it)
;oplot, [-0.5,10.0], trend[0]*[-0.5,10.0]+trend[1], linestyle=0, thick=4.
;p=trend[0]
;q=trend[1]
...
;oplot, [-0.5,10.0], p*[-0.5,10.0]+q-3*s[1], linestyle=1
xyouts, 1.00,20.5,'p='+strtrim(string(p, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(trend[2], FORMAT='(F5.2)'),2), color=cc[0], size=1.3
oplot, [0.85,0.95],[20.45,20.45], linestyle=0, thick=4.
...
y=[y1,y2,y3]
x=[x1,x2,x3]
oplot, x,y , psym=5, color=cc[30]
item=[item,'-20<M!D814!N<-18']
lstyle=[lstyle,1]
sym= [sym,5]
...
fit1[0]=1/fit1[0]
q=(fit[0]*fit1[1]-fit1[0]*fit[1])/(fit[0]-fit1[0])-p*(fit1[1]-fit[1])/(fit[0]-fit1[0])
;oplot, [-0.5,10.0], p*[-0.5,10.0]+q, linestyle=1, thick=4.
del=abs(p*x+q-y)
s=stdev(del)
;oplot, [-0.5,10.0], p*[-0.5,10.0]+q+2*s[1], linestyle=2
;oplot, [-0.5,10.0], p*[-0.5,10.0]+q-2*s[1], linestyle=2
...
oplot, [-0.5,10.0], p*[-0.5,10.0]+q, linestyle=2, thick=4.
print,   p, q, trend[2], trend[3]
xyouts, 1.00,21,'p='+strtrim(string(p, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(trend[2], FORMAT='(F5.2)'),2), color=cc[0], size=1.3
oplot, [0.85,0.95],[20.95,20.95], linestyle=1, thick=4.
...
y=[y1,y2,y3]
x=[x1,x2,x3]
oplot, x,y , psym=4, color=cc[50]
item=[item,'-18<M!D814!N<-16']
lstyle=[lstyle,1]
sym= [sym,4]
c_colors=[c_colors,50]
...
y=[y1]
x=[x1]
oplot, x,y , psym=2, color=cc[8]
item=[item,'-16<M!D814!N']
lstyle=[lstyle,1]
sym= [sym,2]
c_colors=[c_colors,8]
xyouts, 1.00,21.5,'p='+strtrim(string(p, FORMAT='(F5.2)'),2)+'!M+!X'+strtrim(string(trend[2], FORMAT='(F5.2)'),2), color=cc[0], size=1.3
oplot, [0.85,0.95],[21.45,21.45], linestyle=2, thick=4.

legend, item, linestyle=lstyle, color=c_colors , psym=sym, /clear, /right, charsize=1.4, box=0
